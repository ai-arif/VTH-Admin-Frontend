...
const oldIndex = () => {
  ...
  useEffect(() => {
    if (router.query?.id) {
      axiosInstance.get(`/patient-registration/incoming/${router.query?.id}`).then((res) => {
  ...
  useEffect(() => {
    if (prescriptions?._id)
      axiosInstance.get(`/test/test-result/${prescriptions?.appointmentId?._id}`).then((response) => {
        let arr = [];
  ...
  const onSubmit = async (testData) => {
    try {
      const data = {
        ...
        appointmentId: prescriptions?.appointmentId?._id,
        phone: prescriptions?.appointmentId?.phone,
            ...
            {results?.includes(testAllInfo?._id) ? (
              <div className="p-3">
                <div className="d-flex justify-content-between align-items-center m-0 border  border-bottom-0 p-1">
                  <h3 className="">Owner: {prescriptions?.appointmentId?.ownerName}</h3>
                  <div className="d-flex align-items-center gap-2">
                    <p>Case No: {prescriptions?.appointmentId?.caseNo}</p>
                    <p>|</p>
                    <p>Date: {new Date(prescriptions?.appointmentId?.createdAt).toDateString()}</p>
                    <p>|</p>
                    <p>
                      Status:{" "}
