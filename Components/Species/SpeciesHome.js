...
import { deleteSpeciesData, fetchSpecies, searchSpeciesData } from "../../features/specie/speciesSlice";
...
const SpeciesHome = () => {
  ...
  const { species, status, totalPages } = useSelector((state) => state.specie);
  ...
  const handleSearch = async () => {
    try {
      if (search.trim()) {
        const res = await dispatch(searchSpeciesData({ search }));
  ...
  useEffect(() => {
    if (router.isReady) {
      dispatch(fetchSpecies({ page: currentPage }));
    }
