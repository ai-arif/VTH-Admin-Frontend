...
const UpdateStaff = ({ existingData }) => {
  ...
  const onSubmit = async (staffData) => {
    try {
      ...
      console.error(error);
      toast.error("An error occurred while updating account. Please try again later.");
    }
  };
          ...
          <div className="modal-body">
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="mb-3">
                ...
                <input type="text" {...register("email", { required: false })} className={`form-control ${errors.phone && "border-danger"}`} />
                {errors.phone && <small className="text-danger">Please write email</small>}
              </div>
